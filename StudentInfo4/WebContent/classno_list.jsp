...
<body>
    <nav class="navbar navbar-default navbar-static-top">
        <div class="navbar-header">
            ...
            <a href="#" class="navbar-brand">班级信息管理</a>
        </div>
            ...
            <div class="col-md-12 col-sm-3">
                <div class="panel panel-default">
				    ...
				    <div style="float: right;margin: 5px;">
				        ...
				        <a class="btn btn-primary" href="${pageContext.request.contextPath}/classno_add.jsp">添加班级</a>
                    ...
                    <form id="form" action="${pageContext.request.contextPath}/classno?method=delSelectedClassNo" method="post">
                        <table class="table table-striped table-hover" style="margin: 10px;">
                            ...
                            <c:forEach items="${pb.list}" var="classNo" varStatus="s">
					            <tr>
					                <td><input type="checkbox" name="id" value="${classNo.id}"></td>
					                <td>${classNo.id}</td> 
					                <td>${classNo.cno}</td>
					                <td>${classNo.className}</td>
					                <td>${classNo.department}</td>
					                <td><a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/classno?method=findClassNo&id=${classNo.id}">修改</a>&nbsp;
					                    <a class="btn btn-default btn-sm" href="javascript:deleteStudent(${classNo.id});">删除</a></td>
					            </tr>
                        ...
                        <nav aria-label="Page navigation" class="pull-right">
                            <ul class="pagination" style="margin-top: 10px;">
				                ...
				                <c:if test="${pb.currentPage != 1}">
				                    ...
				                    <a href="${pageContext.request.contextPath}/classno?method=findAll&currentPage=${pb.currentPage - 1}&rows=5&name=${condition.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}" aria-label="Previous">
				                ...
				                <c:forEach begin="1" end="${pb.totalPage}" var="i" >
				                    ...
				                    <c:if test="${pb.currentPage == i}">
				                        <li class="active"><a href="${pageContext.request.contextPath}/classno?method=findAll&currentPage=${i}&rows=5&name=${condition.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}">${i}</a></li>
				                    </c:if>
				                    <c:if test="${pb.currentPage != i}">
				                        <li><a href="${pageContext.request.contextPath}/classno?method=findAll&currentPage=${i}&rows=5&name=${condition.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}">${i}</a></li>
				                    </c:if>
				
				                </c:forEach>
				
				                <li>
				                    <a href="${pageContext.request.contextPath}/classno?method=findAll&currentPage=${pb.currentPage + 1}&rows=5&name=${condition.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}" aria-label="Next">
